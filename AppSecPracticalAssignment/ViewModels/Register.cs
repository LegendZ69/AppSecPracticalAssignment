...
namespace AppSecPracticalAssignment.ViewModels
{
    public class Register
    {
        ...
        [RegularExpression(@"^[a-zA-Z]+$", ErrorMessage = "First Name must be letters")]
        /*[RegularExpression(@"^[a-zA-Z-' ]+$", ErrorMessage = "First Name must be letters or (-' )")]*/
        [DataType(DataType.Text)]
        public string FirstName { get; set; }

        [Required(ErrorMessage = "Last Name is required")]
        [RegularExpression(@"^[a-zA-Z]+$", ErrorMessage = "Last Name must be letters")]
        ...
        */
        ...
        [RegularExpression(@"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^\w\d\s])[^\s]{12,64}$")]
